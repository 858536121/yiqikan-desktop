...
import { resolveEffectiveExtensionPath, checkForExtensionUpdate } from "./extension-updater.js";
...
let currentLoadedExtensionVersion: string | null = null;
...
ipcMain.handle("yiqikan:get-loaded-extension-version", () => {
  return currentLoadedExtensionVersion;
});

ipcMain.handle("yiqikan:relaunch", () => {
  console.info("[Desktop] 收到前端重启请求，准备重启应用以生效新扩展...");
  app.relaunch();
  app.exit(0);
});
...
app.whenReady().then(async () => {
  ...
  // Load browser extension (prioritizes hot-updated active directory, fallbacks to packaged/dev)
  const builtinPath = getBuiltinExtensionPath();
  const extPath = resolveEffectiveExtensionPath(builtinPath);
  if (extPath) {
    try {
      const ext = await session.defaultSession.loadExtension(extPath, { allowFileAccess: true });
      currentLoadedExtensionVersion = ext.version;
      console.info(`[Desktop] 浏览器扩展加载成功: ${ext.name} (v${ext.version}) 路径: ${extPath}`);
    } catch (err) {
      console.error("[Desktop] 加载浏览器扩展失败:", err);
    }
  } else {
    console.warn("[Desktop] 未找到可用浏览器扩展路径");
  }

  // Asynchronously check for extension OTA hot updates in background
  setTimeout(async () => {
    try {
      const updateResult = await checkForExtensionUpdate({ fallbackBuiltinPath: builtinPath });
      if (updateResult.updated && updateResult.targetVersion) {
        console.info(`[Desktop Extension OTA] 扩展热更新就绪: v${updateResult.targetVersion}，通知前端提示重启`);
        if (currentMainWindow && !currentMainWindow.isDestroyed()) {
          currentMainWindow.webContents.send("yiqikan:extension-update-ready", {
      ...
      console.warn("[Desktop Extension OTA] 后台检查扩展更新出错:", err);
    }
  }, 4000);
