...
// Returns the active hot renderer version — userData takes priority over bundled
async function readHotVersion(): Promise<string | null> {
  for (const p of [hotVersionPath, bundledHotVersionPath]) {
    try {
      const v = await readFile(p, "utf-8");
...
// Bundled renderer shipped inside the app package (resources/renderer-hot/)
const bundledHotDir = join(__dirname, "../../renderer-hot");
const bundledHotIndexPath = join(bundledHotDir, "index.html");
const bundledHotVersionPath = join(bundledHotDir, "version.txt");

// Returns path to hot renderer index.html — userData takes priority over bundled
export function getHotRendererPath(): string | null {
  if (existsSync(hotIndexPath)) return hotIndexPath;
  if (existsSync(bundledHotIndexPath)) return bundledHotIndexPath;
  return null;
}

// Returns the active hot renderer version synchronously — userData takes priority over bundled
export function getHotRendererVersion(): string | null {
  try {
    const { readFileSync } = require("node:fs") as typeof import("node:fs");
    for (const p of [hotVersionPath, bundledHotVersionPath]) {
      try {
        const v = readFileSync(p, "utf-8");
        if (v.trim()) return v.trim();
      } catch { /* try next */ }
    }
    return null;
...
function resolveFullUrl(rawUrl: string, baseUrl: string | null): string {
  if (!rawUrl) return "";
...
async function downloadRendererZip(url: string): Promise<void> {
  const targetUrl = resolveFullUrl(url, releaseConfigUrl);
...
async function runCheck() {
  ...
  if (hotVersion && compareVersions(hotVersion, config.rendererVersion) >= 0) {
    setRendererState({
      status: "up-to-date",
  ...
  try {
    ...
    setRendererState({
      ...
      availableVersion: config.rendererVersion,
      progressPercent: 100,
      message: `新版本 ${config.rendererVersion} 已就绪，点击刷新即可生效`,
      error: null,
    });
  } catch (err) {
...
export function registerReleaseUpdaterIpc() {
  ...
  ipcMain.handle("yiqikan:get-hot-renderer-version", () => getHotRendererVersion());
  ipcMain.handle("yiqikan:apply-renderer-update", async () => {
    // Reset state so the reloaded renderer doesn't show the update banner again
    const appliedVersion = rendererUpdateState.availableVersion ?? rendererUpdateState.currentVersion;
    setRendererState({
      status: "up-to-date",
    ...
    for (const win of BrowserWindow.getAllWindows()) {
      if (!win.isDestroyed()) {
        // Clear cached JS/CSS so the new renderer bundle is actually loaded
        await win.webContents.session.clearCache();
        if (hotPath) {
          // Append timestamp to bypass aggressive memory caching of index.html
          win.loadFile(hotPath, { query: { t: Date.now().toString() } }).catch(() => {});
        } else {
...
export async function initializeReleaseUpdater() {
  ...
  const defaultUrl = "https://yiqikan.club/api/release-config";
  ...
  // Initial check after short delay, then repeat on interval
  setTimeout(() => {
    runCheck().catch(() => {});
    setInterval(() => { runCheck().catch(() => {}); }, intervalMs);
  }, delayMs);
}
