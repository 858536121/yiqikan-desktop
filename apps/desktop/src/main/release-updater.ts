...
async function runCheck() {
  ...
  const installedVersion = hotVersion ?? "0.0.0";

  if (compareVersions(installedVersion, config.rendererVersion) >= 0) {
    setRendererState({
      status: "up-to-date",
...
export async function initializeReleaseUpdater() {
  ...
  const enabled = readBooleanFlag(process.env.YIQIKAN_RELEASE_UPDATE_ENABLED, false);
  const url = readOptionalUrl(process.env.YIQIKAN_RELEASE_CONFIG_URL);
  ...
  if (!enabled || !url) {
    setRendererState({ status: "disabled", message: !enabled ? "热更未启用" : "未配置热更地址" });
    return;
  }
