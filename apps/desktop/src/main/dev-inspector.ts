...
export function startDevInspector(yiqikanSession: Session) {
  ...
  server.on("error", (error: NodeJS.ErrnoException) => {
    if (error.code === "EADDRINUSE") {
      console.warn(`[yiqikan dev] inspector port ${PORT} is already in use; skipping inspector for this instance`);
      return;
    }
    console.warn("[yiqikan dev] inspector failed to start:", error);
  });
}
