...
export function useRoomSocket({
  ...
  useEffect(() => {
    ...
    const handleError = (payload: ServerErrorPayload) => {
      // Ignore HOST_ONLY during the brief window after reconnect where isHostRef
      // hasn't updated yet and a stale video-status report fires a PlayerEvent
      if (payload.code === "HOST_ONLY") return;
