...
export function useVideoSync({
  ...
  function shouldBlockLocalResume(): boolean {
    if (!roomRef.current || isHostRef.current) return false;
    const effectivePaused = authoritativePlaybackPausedRef.current === null ? true : authoritativePlaybackPausedRef.current;
    return effectivePaused === true && !hasAuthoritativeSyncEvent(false) && !hasMemberSyncIntent(false);
  }
  ...
  useEffect(() => {
    ...
    function handleIpcMessage(event: any) {
      ...
      if (channel === "yiqikan:video-status") {
        ...
        if (nextStatus.found) {
          ...
          if (!hasDetectedVideoRef.current) {
            log("info", "VideoSync", `Video detected on page: ${(nextStatus as any).videoTitle || activeUrl}`, {
          ...
          if (isHostRef.current && activeRoom && currentUserId) {
            ...
            if (isUserAction || isSignificantDrift || heartbeatDue) {
              ...
              log("info", "VideoSync", `[Host Broadcast] ct=${nextStatus.currentTime?.toFixed(1)} paused=${nextStatus.paused} rate=${nextStatus.playbackRate}`);
      ...
      if (channel === "yiqikan:play-attempt") {
        const activeRoom = roomRef.current;
        if (!activeRoom || isHostRef.current) return;
  ...
  useEffect(() => {
    function handleVideoSync(payload: PlayerEventPayload) {
      ...
      const wv = webviewRef.current;
      if (!wv) return;
      log("info", "VideoSync", `[Member Apply] ct=${payload.currentTime?.toFixed(1)} paused=${payload.paused} rate=${payload.playbackRate}`);
