...
export function useRoomActions({
  ...
  const createRoom = useCallback((options: CreateRoomOptions = {}) => {
    ...
    const targetRoomId = normalizeRoomIdInput(options.roomId ?? roomCode) || undefined;
    const hasPwd = Boolean(options.password !== null && (options.password ?? password).trim());

    desktopTelemetry.trackRoomCreate(targetRoomId || "auto", false, hasPwd);

    client.emit(ROOM_EVENTS.CreateRoom, {
      roomId: options.roomId === null ? undefined : targetRoomId,
      userName: name,
      password: options.password === null ? undefined : (options.password ?? password).trim() || undefined,
      initialPlayback,
    });
  }, [client, name, normalizeUrl, pageTitle, password, preRoomUrl, roomCode, urlInput, videoStatusRef, webviewRef]);

  const joinRoom = useCallback(() => {
    const targetRoomId = normalizeRoomIdInput(roomCode);
    desktopTelemetry.trackRoomJoin(targetRoomId);
  ...
  const leaveRoom = useCallback(() => {
    ...
    desktopTelemetry.trackRoomLeave(room.id);
  ...
  const updateMemberName = useCallback((userName: string) => {
    if (!room) return;
    client.emit(ROOM_EVENTS.UpdateMemberName, {
      roomId: room.id,
      userName,
    } satisfies UpdateMemberNamePayload);
    showToast("昵称已更新");
  }, [client, room, showToast]);

  const kickMember = useCallback((targetId: string) => {
    if (!room || !isHost) return;
    client.emit(ROOM_EVENTS.KickMember, {
      roomId: room.id,
      targetId,
    } satisfies KickMemberPayload);
  }, [client, isHost, room]);

  const transferHost = useCallback((targetId: string) => {
    if (!room || !isHost) return;
    client.emit(ROOM_EVENTS.TransferHost, {
      roomId: room.id,
      targetId,
    } satisfies TransferHostPayload);
  }, [client, isHost, room]);
