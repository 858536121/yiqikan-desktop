...
export function useWebviewNavigation({
  ...
  const preRoomUrlRef = useRef("");
  const lastLoadUrlRef = useRef<string | null>(null);
  ...
  useEffect(() => {
    preRoomUrlRef.current = preRoomUrl;
  }, [preRoomUrl]);

  useEffect(() => {
    fetch("https://yiqikan.club/api/search-config", { cache: "no-store" })
      .then((res) => res.json())
  ...
  const normalizeUrl = useCallback((value: string) => {
    ...
    // Otherwise treat as a search query with dynamic template
  ...
  const safeLoadUrl = useCallback((targetUrl: string) => {
    ...
    try {
      ...
      const duplicatePendingLoad = lastLoadUrlRef.current === targetUrl && Date.now() - lastLoadAtRef.current < 2000;
      if (currentUrl === targetUrl || duplicatePendingLoad) return;
      lastLoadUrlRef.current = targetUrl;
  ...
  const openUrlInCurrentView = useCallback((targetUrl: string) => {
    ...
    if (webviewReady) {
      if (!activeRoom) setPreRoomUrl(normalizedUrl);
      persistLastViewedUrl(normalizedUrl);
      safeLoadUrl(normalizedUrl);
    } else if (!activeRoom) {
      persistLastViewedUrl(normalizedUrl);
      setPreRoomUrl(normalizedUrl);
      // If webview exists but dom-ready was missed, try loading directly
      const wv = webviewRef.current;
      if (wv) {
        try {
          wv.getURL(); // throws if not ready
          safeLoadUrl(normalizedUrl);
        } catch {
          // genuinely not ready yet, src attribute change will trigger load
        }
      }
    }
  }, [emitPlayerEvent, isHostRef, normalizeUrl, persistLastViewedUrl, roomRef, safeLoadUrl, setUrlInput, showToast, webviewReady, webviewRef]);
  ...
  useEffect(() => {
    ...
    function tryAttach() {
      ...
      function handleNavigated() {
        ...
        if (!nextUrl || nextUrl === "about:blank") return;
        setUrlInput(nextUrl);
        if (!roomRef.current && !preRoomUrlRef.current) setPreRoomUrl(nextUrl);
      ...
      function handleStopLoading() {
        ...
        if (!activeRoom && !preRoomUrlRef.current) setPreRoomUrl(nextUrl);
        persistLastViewedUrl(nextUrl);
      ...
      // dom-ready may have already fired before we attached — detect by checking if getURL() works
      try {
        const existingUrl = view.getURL();
        if (existingUrl !== undefined) {
          // webview is already ready
          handleDomReady();
        ...
        // not ready yet, dom-ready event will fire normally
      }
  ...
  }, [emitPlayerEvent, isHostRef, persistLastViewedUrl, roomRef, setUrlInput, webviewRef]);

  useEffect(() => {
    const webview = webviewRef.current;
    const nextUrl = room?.playback.url;
    if (!webview || !webviewReady || !nextUrl || isHostRef.current) return;
  ...
  }, [isHostRef, room?.id, room?.playback.url, safeLoadUrl, webviewReady, webviewRef]);

  useEffect(() => {
    if (room || !webviewReady || !preRoomUrl) return;
    safeLoadUrl(preRoomUrl);
  }, [preRoomUrl, room, safeLoadUrl, webviewReady]);
  ...
  return {
    ...
    goHome: () => {
      setPreRoomUrl("");
      setUrlInput("");
      // Stop current page without adding about:blank to history
      const wv = webviewRef.current;
      if (wv) {
    ...
    openLastUrl: (url: string) => {
      // Navigate directly — webview is always mounted now
      openUrlInCurrentView(url);
    },
    lastUrl,
    lastTitle,
    loading,
