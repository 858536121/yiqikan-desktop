...
import type { RendererUpdateState, ShellUpdateState } from "../../shared/renderer-update";
...
const desktopBridge = (window as any).yiqikan ?? {
  ...
  getShellUpdateState: () => Promise.resolve({ status: "none", message: null } as ShellUpdateState),
...
export function App() {
  ...
  const [shellUpdateState, setShellUpdateState] = useState<ShellUpdateState>({ status: "none", message: null });
  ...
  useEffect(() => {
    desktopBridge.getShellUpdateState().then(setShellUpdateState).catch(() => {});
    return desktopBridge.onShellUpdateState(setShellUpdateState);
  }, []);
  ...
  return (
    <div className="flex h-screen overflow-hidden">
      {/* Shell forced-update overlay — blocks UI entirely */}
      {shellUpdateState.status === "forced" && (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/80 backdrop-blur-sm">
          <div className="mx-4 max-w-sm rounded-2xl bg-zinc-900 border border-white/10 p-6 text-center shadow-2xl">
            <div className="text-2xl mb-3">🔄</div>
            <h2 className="text-white font-bold text-lg mb-2">需要更新</h2>
            <p className="text-zinc-400 text-sm leading-6 mb-5">
              {shellUpdateState.message ?? "当前版本过旧，请更新后继续使用。"}
            </p>
            <button
              onClick={() => desktopBridge.checkForAppUpdates().catch(() => {})}
              className="w-full rounded-xl bg-orange-500 hover:bg-orange-400 text-white font-semibold py-2.5 text-sm transition-colors"
            >
              立即更新
            </button>
          </div>
      ...
      {/* Shell suggested-update banner — dismissible */}
      {shellUpdateState.status === "suggested" && (
        <div className="fixed top-3 left-1/2 -translate-x-1/2 z-[9998] flex items-center gap-3 rounded-full bg-zinc-800/90 backdrop-blur-md border border-white/10 px-4 py-2 shadow-lg text-sm text-white">
          <span>{shellUpdateState.message ?? "发现新版本"}</span>
          <button
            onClick={() => desktopBridge.checkForAppUpdates().catch(() => {})}
            className="rounded-full bg-orange-500 hover:bg-orange-400 text-white text-xs font-semibold px-3 py-1 transition-colors"
          >
            更新
          </button>
          <button
            onClick={() => desktopBridge.dismissShellUpdate().then(() => setShellUpdateState({ status: "none", message: null })).catch(() => {})}
