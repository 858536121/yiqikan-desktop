...
import { desktopTelemetry } from "./lib/telemetry";
...
const DESKTOP_AUTH_STORAGE_KEY = "yiqikan:desktopAuth";
const DEFAULT_WEB_URL = "https://yiqikan.club";
...
type DesktopAuthUser = { id: string; email: string; nickname: string };
type DesktopAuthState = { token: string; user: DesktopAuthUser };

function resolveWebUrl() {
  return (import.meta.env.VITE_YIQIKAN_WEB_URL?.trim() || DEFAULT_WEB_URL).replace(/\/+$/, "");
}

function readDesktopAuth(): DesktopAuthState | null {
  try {
    const raw = localStorage.getItem(DESKTOP_AUTH_STORAGE_KEY);
    if (!raw) return null;
    const data = JSON.parse(raw) as DesktopAuthState;
...
function createDeviceCode() {
  if (typeof crypto !== "undefined" && typeof crypto.randomUUID === "function") {
    return `desktop-auth-${crypto.randomUUID()}`;
  }
  return `desktop-auth-${Date.now().toString(36)}-${Math.random().toString(36).slice(2)}`;
}
...
export function App() {
  ...
  const [roomInfoCollapsed, setRoomInfoCollapsed] = useState(() => {
    if (typeof window === "undefined") return false;
    return localStorage.getItem("yiqikan:roomInfoCollapsed") === "1";
  });
  ...
  const [desktopAuth, setDesktopAuth] = useState<DesktopAuthState | null>(() => readDesktopAuth());
  const [desktopAuthStatus, setDesktopAuthStatus] = useState<"idle" | "waiting">("idle");
  ...
  const {
    ...
    isDragging: collapsedToggleDragging,
    buttonRef: collapsedToggleButtonRef,
    suppressClickRef: suppressCollapsedToggleClickRef,
    handlePointerDown: handleCollapsedTogglePointerDown,
  ...
  useEffect(() => {
    ...
    desktopTelemetry.setRoomState(Boolean(room?.id), room?.id);
  }, [isHost, room]);

  useEffect(() => {
    desktopTelemetry.init({
      webUrl: resolveWebUrl(),
      appVersion: runtimeContext.client.appVersion,
      rendererVersion: hotVersion,
      userId: desktopAuth?.user?.id,
    });
  }, [hotVersion, runtimeContext.client.appVersion]);

  useEffect(() => {
    desktopTelemetry.setUserId(desktopAuth?.user?.id ?? null);
  }, [desktopAuth?.user?.id]);
  ...
  const startDesktopLogin = useCallback(() => {
    ...
    const authUrl = `${webUrl}/auth/desktop?deviceCode=${encodeURIComponent(deviceCode)}`;
    setDesktopAuthStatus("waiting");
    desktopBridge.openExternal(authUrl).catch(() => {
      showToast("无法打开登录页面");
      setDesktopAuthStatus("idle");
    });
    
    // Auto reset waiting status after 5 minutes
    setTimeout(() => {
      setDesktopAuthStatus((prev) => (prev === "waiting" ? "idle" : prev));
    }, 5 * 60 * 1000);
  }, [showToast]);

  const logoutDesktopAuth = useCallback(() => {
  ...
  useEffect(() => {
    if (!room) return;
    setPublicTitle((current) => current || room.publicMeta?.title || pageTitle || room.playback.pageTitle || "一起看电影");
  }, [pageTitle, room]);

  const updatePublicRoom = useCallback((isPublic: boolean) => {
    if (!room || !isHost) return;
    if (isPublic && !desktopAuth?.token) {
      startDesktopLogin();
      return;
    }
    socketClient.emit(ROOM_EVENTS.UpdatePublicRoom, {
      roomId: room.id,
      isPublic,
      title: publicTitle.trim() || pageTitle || room.playback.pageTitle || "一起看电影",
      authToken: desktopAuth?.token,
    });
  }, [desktopAuth?.token, isHost, pageTitle, publicTitle, room, socketClient, startDesktopLogin]);
  ...
  // Deep link: yiqikan://join/<roomId>?password=<pwd> or yiqikan://create
  useEffect(() => {
    return desktopBridge.onDeepLink((url: string) => {
      try {
        ...
        if (parsed.hostname === "auth") {
          ...
          if (token && userStr) {
            try {
              ...
              localStorage.setItem(DESKTOP_AUTH_STORAGE_KEY, JSON.stringify(nextAuth));
              setDesktopAuth(nextAuth);
              setName((current) => current.trim() ? current : user.nickname);
              setDesktopAuthStatus("idle");
              showToast("已登录，可以公开观影了");
            } catch {
              showToast("登录数据解析失败");
            }
          } else {
        ...
        if (parsed.hostname === "create") {
          if (roomRef.current) {
            showToast("请先退出当前房间再创建新房间");
            return;
          }
          setRoomCode("");
          setPassword("");
          createRoom({ roomId: null, password: null });
          showToast("正在创建房间");
          return;
        }
        const nextRoomId = (parsed.hostname || parsed.pathname.replace(/^\/+/, "")).trim().toLowerCase();
        if (!nextRoomId) return;
        const nextPassword = parsed.searchParams.get("password") || undefined;
        if (roomRef.current) {
          showToast("请先退出当前房间再加入新房间");
          return;
        }
        setRoomCode(nextRoomId);
  ...
  const copyInviteLink = useCallback(() => {
    if (!room) return;
    const base = "https://yiqikan.club/join";
  ...
  return (
    <div className="flex h-screen overflow-hidden">
      ...
      <BrowserPane
        ...
        collapsedToggleDragging={collapsedToggleDragging}
        suppressCollapsedToggleClickRef={suppressCollapsedToggleClickRef}
        onCollapsedTogglePointerDown={handleCollapsedTogglePointerDown}
      ...
      {!collapsed && (
        <RightSidebar
          ...
          onStartDesktopLogin={startDesktopLogin}
          onLogoutDesktopAuth={logoutDesktopAuth}
          onPublicTitleChange={setPublicTitle}
          onUpdatePublicRoom={updatePublicRoom}
          ...
          onToggleRoomInfoCollapsed={() => {
            setRoomInfoCollapsed((value) => {
              const next = !value;
              if (typeof window !== "undefined") {
                localStorage.setItem("yiqikan:roomInfoCollapsed", next ? "1" : "0");
              }
              return next;
            });
          }}
