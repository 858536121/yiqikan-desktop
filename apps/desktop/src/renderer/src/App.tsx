...
export function App() {
  ...
  const desktopAuthPollTimerRef = useRef<number | null>(null);
  ...
  useEffect(() => () => {
    if (desktopAuthPollTimerRef.current) {
      window.clearInterval(desktopAuthPollTimerRef.current);
      desktopAuthPollTimerRef.current = null;
    }
  }, []);
  ...
  const startDesktopLogin = useCallback(() => {
    ...
    let attempts = 0;
    desktopAuthPollTimerRef.current = window.setInterval(async () => {
      attempts += 1;
      if (attempts > 150) {
        if (desktopAuthPollTimerRef.current) {
          window.clearInterval(desktopAuthPollTimerRef.current);
        ...
        setDesktopAuthStatus("idle");
        showToast("登录授权已超时，请重试");
        return;
      }
      try {
        const response = await fetch(`${webUrl}/api/auth/desktop/poll?deviceCode=${encodeURIComponent(deviceCode)}`, { cache: "no-store" });
        if (!response.ok) return;
        const data = await response.json() as { status?: string; token?: string; user?: DesktopAuthUser };
        if (data.status !== "authorized" || !data.token || !data.user) return;
        ...
        setName((current) => current.trim() ? current : data.user!.nickname);
        setDesktopAuthStatus("idle");
        if (desktopAuthPollTimerRef.current) {
          window.clearInterval(desktopAuthPollTimerRef.current);
          desktopAuthPollTimerRef.current = null;
