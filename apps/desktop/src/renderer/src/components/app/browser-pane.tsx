...
interface BrowserPaneProps {
  ...
  collapsedToggleDragging?: boolean;
  suppressCollapsedToggleClickRef: MutableRefObject<boolean>;
  onCollapsedTogglePointerDown?: (event: React.PointerEvent<HTMLButtonElement>) => void;
...
export function BrowserPane({
  ...
  collapsedToggleDragging,
  suppressCollapsedToggleClickRef,
  onCollapsedTogglePointerDown,
  setCollapsed,
  canGoBack,
  canGoForward,
  webviewReady,
  ...
  const [lampFaded, setLampFaded] = useState(false);
  const lampFadeTimerRef = useRef<ReturnType<typeof setTimeout> | null>(null);

  useEffect(() => {
    if (!collapsed) {
      setLampFaded(false);
      if (lampFadeTimerRef.current) clearTimeout(lampFadeTimerRef.current);
      return;
    }
    setLampFaded(false);
    lampFadeTimerRef.current = setTimeout(() => setLampFaded(true), 5000);
    return () => {
      if (lampFadeTimerRef.current) clearTimeout(lampFadeTimerRef.current);
    };
  }, [collapsed]);

  function handleLampMouseEnter() {
    if (lampFadeTimerRef.current) clearTimeout(lampFadeTimerRef.current);
    setLampFaded(false);
  }

  function handleLampMouseLeave() {
    if (lampFadeTimerRef.current) clearTimeout(lampFadeTimerRef.current);
    lampFadeTimerRef.current = setTimeout(() => setLampFaded(true), 5000);
  }
    ...
    <>
      <div className="flex-1 flex flex-col min-w-0">
              ...
              >
                ...
                >
                  ...
                  <input
                    ...
                    placeholder={room && !isHost ? "等待房主导航…" : "输入网址或搜索，如 bilibili.com"}
                ...
                <Button type="submit" disabled={!canNavigate || !urlInput.trim()} className="shrink-0 h-8 px-3 text-xs">
                  {looksLikeUrl(urlInput) ? <Send className="w-3.5 h-3.5" /> : <Search className="w-3.5 h-3.5" />}
                  <span>{looksLikeUrl(urlInput) ? "前往" : "搜索"}</span>
                </Button>
              </form>
              ...
              <div className="relative shrink-0">
                ...
                {menuOpen && (
                  <>
                    ...
                    <div className="absolute right-0 top-full mt-1 z-50 min-w-[150px] overflow-hidden rounded-xl border border-white/10 bg-[#0e0e10]/95 shadow-2xl backdrop-blur-md py-1">
                      <button
                        className="w-full px-3.5 py-2 text-left text-xs text-zinc-300 hover:text-white hover:bg-white/[0.08] transition-colors flex items-center gap-2 cursor-pointer"
                        onClick={() => {
                          setMenuOpen(false);
                          onExportLogs();
                        }}
        ...
        <div className="relative flex-1 min-h-0 bg-[#0a0a0b]">
          {webviewPreloadPath !== null && (
            <div className={cn("absolute inset-0", activeUrl ? "visible" : "invisible")}>
              <webview
                ref={(node) => {
      ...
      {collapsed && (
        <button
          ...
          onPointerDown={onCollapsedTogglePointerDown}
          onClick={() => {
          ...
          className={cn(
            "fixed z-50 flex h-12 w-12 cursor-grab select-none items-center justify-center rounded-2xl border border-orange-500/20 bg-surface/90 text-white shadow-lg backdrop-blur-md hover:scale-105 active:cursor-grabbing",
            collapsedToggleDragging ? "scale-110 shadow-2xl border-orange-500/50 cursor-grabbing ring-2 ring-orange-500/20" : "",
            lampFaded ? "opacity-15 transition-opacity duration-500" : "opacity-100 transition-opacity duration-500",
          )}
...
function looksLikeUrl(value: string) {
  ...
  if (trimmed.startsWith("//") || /^[a-zA-Z][a-zA-Z\d+\-.]*:/.test(trimmed)) return true;
