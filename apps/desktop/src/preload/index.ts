...
import type { RendererUpdateState, ShellUpdateState } from "../shared/renderer-update.js";
...
contextBridge.exposeInMainWorld("yiqikan", {
  ...
  getHotRendererVersion: () => ipcRenderer.invoke("yiqikan:get-hot-renderer-version") as Promise<string | null>,
  applyRendererUpdate: () => ipcRenderer.invoke("yiqikan:apply-renderer-update") as Promise<void>,
  getRendererUpdateState: () => ipcRenderer.invoke("yiqikan:get-renderer-update-state") as Promise<RendererUpdateState>,
  getShellUpdateState: () => ipcRenderer.invoke("yiqikan:get-shell-update-state") as Promise<ShellUpdateState>,
  checkReleaseUpdate: () => ipcRenderer.invoke("yiqikan:check-release-update") as Promise<void>,
  dismissShellUpdate: () => ipcRenderer.invoke("yiqikan:dismiss-shell-update") as Promise<void>,
  onRendererUpdateState: (listener: (payload: RendererUpdateState) => void) => {
    const wrapped = (_event: unknown, payload: RendererUpdateState) => listener(payload);
    ipcRenderer.on("yiqikan:renderer-update-state", wrapped);
    return () => ipcRenderer.removeListener("yiqikan:renderer-update-state", wrapped);
  },
  onShellUpdateState: (listener: (payload: ShellUpdateState) => void) => {
    const wrapped = (_event: unknown, payload: ShellUpdateState) => listener(payload);
    ipcRenderer.on("yiqikan:shell-update-state", wrapped);
    return () => ipcRenderer.removeListener("yiqikan:shell-update-state", wrapped);
  },
});
