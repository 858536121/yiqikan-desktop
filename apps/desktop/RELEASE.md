...
#    或者用一条命令全自动完成步骤 1-3：
ADMIN_URL=https://your-domain.com ADMIN_USER=admin ADMIN_PASS=xxx pnpm release:renderer
git add apps/web/public/releases/
...
路径：`https://your-domain.com/admin` → 更新策略管理
...
| `rendererUrl` | renderer zip 的完整下载地址，格式：`https://your-domain.com/releases/renderer-x.x.x.zip` |
...
| `YIQIKAN_RELEASE_CONFIG_URL` | release-config API 地址 | `https://your-domain.com/api/release-config` |
