...
function runMaestroFlow(flowFile, label) {
  return new Promise((resolve, reject) => {
    ...
    const child = spawn(MAESTRO_BIN, ['test', flowFile], {
      stdio: 'inherit',
      env: {
        ...process.env,
        MAESTRO_DRIVER_STARTUP_TIMEOUT: process.env.MAESTRO_DRIVER_STARTUP_TIMEOUT || '60000',
      },
    });
    child.on('close', (code) => {
      if (code !== 0) {
        return reject(new Error(`Maestro Flow 执行未通过: ${flowFile} (退出码 ${code})`));
      }
...
async function getRoomIdFromClipboard() {
  try {
  ...
  // 若模拟器跨进程剪贴板未及时同步至宿主机，从本地服务活跃房间列表动态兜底获取
  try {
  ...
  throw new Error('无法从剪贴板或服务端获取目标房间号');
}
...
async function run() {
  ...
  try {
    // 确保模拟器上的 Expo Go 已连接 Metro 并处于就绪状态
    console.log('📱 [前置检查] 确保 Expo Go 应用已加载就绪...');
    try {
      execSync('xcrun simctl pbcopy booted <<< ""', { stdio: 'ignore' });
    } catch {}
    ...
    await runMaestroFlow(path.join(FLOWS_DIR, '01-create-and-share.yaml'), '创建房间与口令分享流程');
    takeScreenshot('01_room_created.png');

    // 从模拟器剪贴板捕获邀请口令
    const roomId = await getRoomIdFromClipboard();
    ...
    await runMaestroFlow(path.join(FLOWS_DIR, '02-verify-and-reply.yaml'), '成员状态验证与弹幕回复流程');
    ...
    // 步骤 4.5: 模拟器端测试实时语音连麦、麦克风静音与降噪回声消除控制
    console.log('\n[步骤 4.5/7] 模拟器端测试实时语音连麦、麦克风静音与降噪回声消除控制...');
    await runMaestroFlow(path.join(FLOWS_DIR, '06-verify-voice.yaml'), '语音连麦与降噪回声控制流程');
    takeScreenshot('03_voice_tested.png');
    console.log('✅ [语音通道验证成功] WebRTC 语音通道握手、麦克风静音与降噪回声控制链路全部就绪');

    // 步骤 5: 视频播放加载与遥控器播控同步
    console.log('\n[步骤 5/7] 模拟器端加载视频，使用遥控器触发快进、暂停与倍速...');
    await runMaestroFlow(path.join(FLOWS_DIR, '04-load-and-control-video.yaml'), '视频加载与遥控器播控流程');
    ...
    await runMaestroFlow(path.join(FLOWS_DIR, '05-return-home.yaml'), '主页导航与状态复原流程');
    takeScreenshot('05_returned_home.png');

    // 步骤 7: 伴侣离开，模拟器端验证人数减少并退出大厅
    console.log('\n[步骤 7/7] 伴侣离开房间，模拟器端验证人数变更并退出...');
    bot.leaveRoom();
    await runMaestroFlow(path.join(FLOWS_DIR, '03-verify-leave-and-exit.yaml'), '退出清理流程');
