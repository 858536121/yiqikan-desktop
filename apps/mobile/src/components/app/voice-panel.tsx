...
export const VoicePanel: React.FC<VoicePanelProps> = ({
  ...
  if (voiceStatus === 'connecting' || voiceStatus === 'reconnecting') {
    return (
      <TouchableOpacity style={styles.compactConnectingPill} onPress={leaveVoice} activeOpacity={0.7} testID="连麦状态胶囊">
        <ActivityIndicator size="small" color="#F97316" style={{ marginRight: 4, transform: [{ scale: 0.75 }] }} />
        <Text style={styles.compactConnectingText}>
          {voiceStatus === 'reconnecting' ? '重连中…' : '连麦中…'}
        </Text>
      </TouchableOpacity>
    );
  }
      ...
      >
        <View style={styles.modalOverlay}>
          <TouchableOpacity 
            ...
            accessibilityLabel="关闭弹窗背景"
          />
          <View style={styles.modalDialog} accessibilityViewIsModal={true} testID="语音设置弹窗">
            <View style={styles.modalHeader}>
              <Text style={styles.modalDialogTitle} testID="语音通话控制标题" accessibilityLabel="语音通话控制">语音通话控制</Text>
              <TouchableOpacity onPress={() => setIsSettingsOpen(false)} style={{ padding: 4 }} testID="语音设置关闭按钮" accessibilityLabel="语音设置关闭按钮">
                <X size={18} color="#888" />
              </TouchableOpacity>
            </View>
...
const styles = StyleSheet.create({
  ...
  compactConnectingPill: {
    ...
    borderLeftColor: 'rgba(249, 115, 22, 0.25)',
  ...
  compactErrorPill: {
    ...
    borderRadius: 10,
    borderWidth: 1,
    borderTopColor: 'rgba(251, 113, 133, 0.45)',
    borderBottomColor: 'rgba(0, 0, 0, 0.30)',
    borderLeftColor: 'rgba(244, 63, 94, 0.25)',
