...
export function ChatPanel({
  ...
  // 独立键盘高度平滑动画追踪，解决 React Native Modal 内部 KeyboardAvoidingView 坐标失效导致输入框不跟手的问题
  ...
  useEffect(() => {
    ...
    const showSub = Keyboard.addListener(showEvent, (e) => {
      // 增加 Android 键盘顶部缓冲间隙 (14px)，彻底消除软键盘顶部与输入框边框的重合
      const extraOffset = Platform.OS === 'android' ? 14 : 0;
      const targetHeight = (e.endCoordinates?.height || 0) + extraOffset;
      Animated.timing(keyboardHeightAnim, {
        toValue: targetHeight,
        duration: Platform.OS === 'ios' ? Math.max(e.duration || 250, 150) : 150,
    ...
    const hideSub = Keyboard.addListener(hideEvent, (e) => {
      Animated.timing(keyboardHeightAnim, {
        toValue: 0,
        duration: Platform.OS === 'ios' ? Math.max(e.duration || 200, 150) : 150,
        useNativeDriver: false,
      }).start();
    });
  ...
  // 弹窗打开后主动对焦，解决 Android 原生 Dialog 挂载时丢失 autoFocus 导致需要点击两次的问题
  useEffect(() => {
    if (isModalOpen) {
