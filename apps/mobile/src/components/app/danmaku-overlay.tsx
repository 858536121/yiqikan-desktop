...
export const DanmakuOverlay = memo(function DanmakuOverlay({ enabled }: DanmakuOverlayProps) {
  ...
  useEffect(() => {
    ...
    newMessages.forEach((msg, idx) => {
      ...
      // 计算弹幕离屏终点，避免多走整屏多余距离导致速度翻倍
      const estimatedBubbleWidth = Math.max(350, (msg.message.length + (msg.actorName?.length || 4) + 4) * 16 + 50);
      const toValue = -estimatedBubbleWidth;

      // 横屏宽度大，适当增加持续时间以保持舒适的阅读速度（横屏约 10s，竖屏约 8s）
      const isLandscape = currentWidth > 500;
      const duration = isLandscape ? 10000 : 8000;
