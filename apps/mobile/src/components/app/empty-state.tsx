...
export const DEFAULT_POPULAR_SITES: RecommendSite[] = [
  ...
  { name: 'Libvio影视', url: 'https://www.libvios.com', color: '#F97316', tag: 'Libvio' },
];
        ...
        <View style={styles.quickNavSection}>
          ...
          <View style={styles.quickNavGrid}>
            {recommendSites.map((site, idx) => (
              <TouchableOpacity
                key={`${site.name}-${idx}`}
                style={[styles.quickNavCard, !canControl && styles.quickNavCardDisabled]}
                disabled={!canControl}
                onPress={() => handleNavigateUrl(site.url)}
                activeOpacity={0.7}
              >
                <View style={[styles.quickNavBadge, { backgroundColor: site.color || '#F97316' }]}>
                  <Text style={styles.quickNavBadgeText}>{site.tag || '影视'}</Text>
                </View>
