...
export function BottomPanel({
  ...
  const [activeTab, setActiveTab] = useState<'remote' | 'chat' | 'members'>(!isInRoom ? (hasVideo ? 'remote' : 'members') : (hasVideo ? 'remote' : 'chat'));
        ...
        <View style={styles.tabGroup}>
          {hasVideo && (
            <TouchableOpacity 
              ...
              activeOpacity={0.7}
            >
              <Tv color={activeTab === 'remote' && !isPanelCollapsed ? '#F97316' : '#888'} size={15} />
...
const styles = StyleSheet.create({
  bottomContainer: {
    height: 280, // 从 350px 适度缩减至 280px，消除大面积黑色空白死区，给视频与内容释放更多视野
