import { NativeModules, NativeEventEmitter, Platform } from 'react-native';
import AsyncStorage from '@react-native-async-storage/async-storage';
import Constants from 'expo-constants';
import * as Updates from 'expo-updates';
import { compareVersions, type AppReleaseConfig, type MobileReleaseConfig } from '@yiqikan/shared';
import { resolveWebUrl } from './environment';

const STORAGE_KEY_ACTIVE_OTA_VERSION = '@yiqikan_ota_active_version';
const STORAGE_KEY_PENDING_OTA_VERSION = '@yiqikan_ota_pending_version';

const NativeOta = NativeModules.YiQiKanOTA;
const otaEventEmitter = NativeOta ? new NativeEventEmitter(NativeOta) : null;

export interface OtaCheckResult {
  status: 'UP_TO_DATE' | 'OTA_UPDATE_AVAILABLE' | 'APP_UPDATE_REQUIRED' | 'ERROR';
  currentVersion: string;
  latestBundleVersion?: string;
  bundleUrl?: string;
  bundleHash?: string;
  updateMode?: 'silent' | 'prompt';
  releaseNotes?: string;
  appDownloadUrl?: string;
  forceAppUpdate?: boolean;
  error?: string;
}

export type DownloadProgressCallback = (percent: number, receivedBytes?: number, totalBytes?: number) => void;

class OtaService {
  private baseAppVersion: string;

  constructor() {
    this.baseAppVersion = Constants.expoConfig?.version || '1.16.0';
  }

  /**
   * 获取当前生效的 UI Bundle 版本号
   * - 若运行在内置底包：严格以底包版本为准
   * - 若成功加载了热更包：读取热更生效版本
   */
  async getCurrentBundleVersion(): Promise<string> {
    try {
      if (Updates.isEnabled) {
        // 如果当前是底包（未加载任何热更包或热更回滚），清理缓存并返回底包版本
        if (Updates.isEmbeddedLaunch) {
          await AsyncStorage.removeItem(STORAGE_KEY_ACTIVE_OTA_VERSION);
          return this.baseAppVersion;
        }

        // 如果不是底包，说明已经成功加载了热更包
        const pending = await AsyncStorage.getItem(STORAGE_KEY_PENDING_OTA_VERSION);
        if (pending && pending.trim()) {
          await AsyncStorage.setItem(STORAGE_KEY_ACTIVE_OTA_VERSION, pending.trim());
          await AsyncStorage.removeItem(STORAGE_KEY_PENDING_OTA_VERSION);
          return pending.trim();
        }

        const active = await AsyncStorage.getItem(STORAGE_KEY_ACTIVE_OTA_VERSION);
        if (active && active.trim()) {
          return active.trim();
        }

        // 尝试从 manifest 获取
        const manifestVersion = (Updates.manifest as any)?.metadata?.version || (Updates.manifest as any)?.extra?.expoClient?.version;
        if (manifestVersion && typeof manifestVersion === 'string') {
          return manifestVersion.trim();
        }
      }
    } catch (e) {
      console.warn('[OTA] 获取当前 Bundle 版本异常:', e);
    }
    return this.baseAppVersion;
  }

  /**
   * 获取原生底包版本号
   */
  getBaseAppVersion(): string {
    return this.baseAppVersion;
  }

  /**
   * 标记当前启动健康进入业务
   */
  async markOtaSuccess(): Promise<void> {
    try {
      if (Platform.OS === 'android' && NativeOta?.markOtaSuccess) {
        await NativeOta.markOtaSuccess();
      }
    } catch {
      // ignore
    }
  }

  /**
   * 检查全平台发布配置与移动端热更新
   */
  async checkUpdate(customWebUrl?: string): Promise<OtaCheckResult> {
    const currentBundleVersion = await this.getCurrentBundleVersion();
    if (__DEV__) {
      return {
        status: 'UP_TO_DATE',
        currentVersion: currentBundleVersion,
      };
    }
    const baseUrl = customWebUrl || resolveWebUrl();

    try {
      // 1. 获取服务端的版本总控配置（用于原生底包强制升级与发布说明）
      const response = await fetch(`${baseUrl.replace(/\/$/, '')}/api/release-config`, {
        headers: { 'Cache-Control': 'no-cache' },
      });

      let mobileConfig: MobileReleaseConfig | undefined = undefined;
      if (response.ok) {
        const releaseConfig = (await response.json()) as AppReleaseConfig;
        mobileConfig = releaseConfig.mobile || undefined;
      }

      // 2. 检查是否需要全量原生 APK 升级（严格限定 Android 平台底包）
      if (
        Platform.OS === 'android' &&
        mobileConfig?.appMinVersion &&
        compareVersions(this.baseAppVersion, mobileConfig.appMinVersion) < 0
      ) {
        const fullApkUrl = mobileConfig.appDownloadUrl?.startsWith('http')
          ? mobileConfig.appDownloadUrl
          : `${baseUrl.replace(/\/$/, '')}${mobileConfig.appDownloadUrl?.startsWith('/') ? '' : '/'}${mobileConfig.appDownloadUrl || 'download'}`;

        return {
          status: 'APP_UPDATE_REQUIRED',
          currentVersion: currentBundleVersion,
          appDownloadUrl: fullApkUrl,
          forceAppUpdate: Boolean(mobileConfig.forceAppUpdate),
          releaseNotes: mobileConfig.releaseNotes || '请更新至最新版 App 以获得完整功能支持',
        };
      }

      // 3. 检查是否有 Expo Updates 热更新可用
      if (Updates.isEnabled) {
        try {
          const updateCheck = await Updates.checkForUpdateAsync();
          if (updateCheck.isAvailable) {
            const targetVersion = mobileConfig?.bundleVersion || (updateCheck.manifest as any)?.metadata?.version || '最新版本';
            return {
              status: 'OTA_UPDATE_AVAILABLE',
              currentVersion: currentBundleVersion,
              latestBundleVersion: targetVersion,
              bundleUrl: mobileConfig?.bundleUrl || (updateCheck.manifest as any)?.launchAsset?.url || 'expo-updates',
              bundleHash: mobileConfig?.bundleHash || undefined,
              updateMode: mobileConfig?.updateMode || 'prompt',
              releaseNotes: mobileConfig?.releaseNotes || '优化了 UI 体验与已知问题',
            };
          }
        } catch (updateErr: any) {
          console.warn('[OTA] 检查 expo-updates 失败:', updateErr?.message || updateErr);
        }
      }

      // 4. 双重保障：若 Updates 检查未命中或失败，但服务端 release-config.mobile.bundleVersion 高于当前生效版本
      if (
        mobileConfig?.bundleVersion &&
        compareVersions(mobileConfig.bundleVersion, currentBundleVersion) > 0
      ) {
        return {
          status: 'OTA_UPDATE_AVAILABLE',
          currentVersion: currentBundleVersion,
          latestBundleVersion: mobileConfig.bundleVersion,
          bundleUrl: mobileConfig?.bundleUrl || 'expo-updates',
          bundleHash: mobileConfig?.bundleHash || undefined,
          updateMode: mobileConfig?.updateMode || 'prompt',
          releaseNotes: mobileConfig?.releaseNotes || '优化了 UI 体验与已知问题',
        };
      }

      return {
        status: 'UP_TO_DATE',
        currentVersion: currentBundleVersion,
      };
    } catch (err: any) {
      return {
        status: 'ERROR',
        currentVersion: currentBundleVersion,
        error: err?.message || '网络连接超时',
      };
    }
  }

  /**
   * 下载并应用热更包（通过 expo-updates 官方管道安全拉取与落盘）
   */
  async downloadAndApply(
    _zipUrl?: string,
    targetVersion?: string,
    _expectedHash?: string,
    onProgress?: DownloadProgressCallback
  ): Promise<boolean> {
    try {
      if (onProgress) {
        onProgress(10, 0, 100);
      }

      if (Updates.isEnabled) {
        if (targetVersion) {
          await AsyncStorage.setItem(STORAGE_KEY_PENDING_OTA_VERSION, targetVersion);
        }
        if (onProgress) onProgress(40, 40, 100);

        const fetchResult = await Updates.fetchUpdateAsync();
        if (onProgress) onProgress(100, 100, 100);

        return fetchResult.isNew;
      }

      // 如果未启用 Updates（如开发模式调试），仅记录版本标记
      if (targetVersion) {
        await AsyncStorage.setItem(STORAGE_KEY_ACTIVE_OTA_VERSION, targetVersion);
      }
      if (onProgress) onProgress(100, 100, 100);
      return true;
    } catch (e: any) {
      console.warn('[OTA] 下载/安装热更包失败:', e);
      throw e;
    }
  }

  /**
   * 应用内下载全量 APK 安装包并自动调起系统安装器
   */
  async downloadAndInstallApk(
    apkUrl: string,
    onProgress?: DownloadProgressCallback
  ): Promise<boolean> {
    if (Platform.OS !== 'android' || !NativeOta?.downloadAndInstallApk) {
      throw new Error('当前平台不支持应用内静默安装 APK');
    }

    let subscription: any = null;

    if (onProgress && otaEventEmitter) {
      subscription = otaEventEmitter.addListener('onApkDownloadProgress', (data: any) => {
        onProgress(data.percent || 0, data.receivedBytes, data.totalBytes);
      });
    }

    try {
      await NativeOta.downloadAndInstallApk(apkUrl);
      return true;
    } catch (e: any) {
      console.warn('[OTA] APK 下载/拉起安装失败:', e);
      throw e;
    } finally {
      if (subscription) {
        subscription.remove();
      }
    }
  }

  /**
   * 手动重新调起已下载 APK 的系统安装器
   */
  async installExistingApk(): Promise<boolean> {
    if (Platform.OS === 'android' && NativeOta?.installExistingApk) {
      await NativeOta.installExistingApk();
      return true;
    }
    return false;
  }

  /**
   * 重载/重启应用使新 Bundle 立即生效
   */
  async reloadApp(): Promise<void> {
    if (Updates.isEnabled) {
      await Updates.reloadAsync();
      return;
    }
    if (Platform.OS === 'android' && NativeOta?.reloadApp) {
      await NativeOta.reloadApp();
    }
  }

  /**
   * 清除热更缓存并回退到底包
   */
  async clearOtaCache(): Promise<void> {
    if (Platform.OS === 'android' && NativeOta?.clearOtaCache) {
      await NativeOta.clearOtaCache();
    }
    await AsyncStorage.removeItem(STORAGE_KEY_ACTIVE_OTA_VERSION);
    await AsyncStorage.removeItem(STORAGE_KEY_PENDING_OTA_VERSION);
  }

  /**
   * 记录已应用的热更版本
   */
  async recordAppliedVersion(version: string): Promise<void> {
    await AsyncStorage.setItem(STORAGE_KEY_ACTIVE_OTA_VERSION, version);
  }
}

export const otaService = new OtaService();
