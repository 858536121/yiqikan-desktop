...
(function() {
  ...
  function getAudioConstraints(enableNC) {
    if (!enableNC) return { echoCancellation: false, noiseSuppression: false };
    return {
  ...
  async function reconnectSFU() {
    ...
    isReconnecting = true;
    log("ICE", "⚠️ ICE 连接中断或失效，正在执行自动重连愈合...");
    ...
    try {
      ...
      await window.__voiceClient.join(lastRoomId, lastUserId, lastUserName);
      log("ICE", "✅ SFU 自动重连愈合成功");
    } catch(err) {
      log("ERROR", "自动重连失败: " + err.message);
      setTimeout(function() {
        if (voiceStatus !== "connected" && lastRoomId) {
          reconnectSFU();
        }
      }, 4000);
    } finally {
      ...
      try {
        ...
        try {
          localStream = await navigator.mediaDevices.getUserMedia({
            audio: getAudioConstraints(noiseCancellation),
            video: false
          });
