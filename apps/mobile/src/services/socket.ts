...
const PROD_SERVER_URL = 'https://together-ws.cpolar.cn';
...
class SocketService {
  ...
  private setupListeners() {
    ...
    this.socket.on('connect', () => {
      ...
      if (this.lastRoomContext) {
        ...
        setTimeout(() => {
          ...
          if (this.socket?.connected && this.lastRoomContext && (!currentRoom || currentRoom.id !== roomId)) {
            console.log('[Socket] 尝试自动重新加入房间恢复会话:', roomId);
    ...
    // 监听房间关闭或被移出房间
    this.socket.on(ROOM_EVENTS.CloseRoom, () => {
      this.lastRoomContext = null;
      useRoomStore.getState().reset();
      useRoomStore.getState().setError('房间已关闭或已被移出房间');
    });

    this.socket.on(ROOM_EVENTS.Error, (error) => {
      useRoomStore.getState().setError(error.message);
      if (error.code === 'ROOM_NOT_FOUND' || error.code === 'KICKED' || error.code === 'INVALID_PASSWORD') {
        this.lastRoomContext = null;
      }
    });
  }
